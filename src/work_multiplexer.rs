...
//! A relatively uncomplicated multiplexer abstraction that allows you to run a
//! parallelizable problem across multiple threads, and signal them to stop if
//! they need to halt early. Eg:
//!
//! ```
...
//!                               break;
//!                           }
//!                           println!("{}: do some work for index {}",
//!                                    name,
//!                                    i);
//!                       }
//!                   });
...
impl ThreadedWorkMultiplexerBuilder {
    ...
    pub fn split_work<F>(self, job: F) -> ThreadedWorkMultiplexerHandles
        where F: Fn(u32, u32, &ThreadNotifier, &str) + Send + Sync + 'static
    {
